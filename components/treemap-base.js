...
function withFadeIn(Component) {
  return (props) => {
    const [animatedProps, set] = useSpring(() => ({ strokeWidth: 0, opacity: 0, config: config.slow }));
...
function TreemapMatrix({width, height, cellWidth, cellHeight, strokeWidth}) {
  ...
  const columnLines = useMemo(() => (
    Array(columns).fill().map((el, colIdx) => {
      return (
        <line
          key={`col${colIdx}`}
          y1="0"
  ...
  return (
    <React.Fragment>
      <rect fill={'var(--primary-fill)'} stroke={'var(--primary-stroke)'} width={width} height={height}></rect>
      {rowLines}
      {columnLines}    
    </React.Fragment>
  )
...
function TreemapSVG({key, width, height, children, ...props}) {
  ...
  useEventListener('mousemove', ({ clientX, clientY }) => {
    ...
    mouseXY.current = dummy.current.matrixTransform(svgEl.current.getScreenCTM().inverse());
  });

  return (
    <svg ref={svgEl} width={width} height={height} {...props}>
      <pattern id="diagonal-stripes" x="0" y="0" width="12" height="12" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
        <rect x="0" y="0" width="10" height="12" stroke="none" fill={'var(--primary-fill)'} />
        <rect x="10" y="0" width="2" height="12" stroke="none" fill={'var(--color-green-90)'} />
      </pattern>
      <pattern id="diagonal-stripes-highlight" x="0" y="0" width="12" height="12" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
        <rect x="0" y="0" width="10" height="12" stroke="none" fill={'var(--primary-fill-hover)'} />
        <rect x="10" y="0" width="2" height="12" stroke="none" fill={'var(--color-green-highlight-90)'} />
      </pattern>
      {children}
      <rect x="0" y="0" width={width} height={height} fill="none" stroke={'var(--primary-stroke)'} strokeWidth={2} />
    </svg>
  )
}
